...
async def send_email_alert_async(subject, body):
    ...
    if not all([EMAIL_DESTINATARIO, EMAIL_REMETENTE, SMTP_API_KEY]):
        print(
            "ERRO DE E-MAIL: Variáveis de ambiente (NOTIFICATION_EMAIL, SENDER_EMAIL, SMTP2GO_API_KEY) não configuradas ou vazias.")
    ...
    try:
        ...
        print(f"ERRO DE E-MAIL (EXCEÇÃO - Conexão): Falha ao conectar ao servidor do SMTP2GO. {e}")
...
async def rodar_simulador():
    global simulated_time_utc
    while True:
        try:
            for _ in range(6):
                if len(data_store) > 0:
                    c_deprec = data_store[-1]["precipitacao_acumulada_mm"]
                else:
                    c_deprec = 0.0
            ...
            print("Tarefa do simulador cancelada."); break


# --- Tarefa de Monitoramento de Alertas (para E-mail) ---
async def monitorar_alertas():
    global global_last_rain_alert_level
    global global_last_soil_alert_level
    ...
    while True:
        try:
            ...
            # Para o monitoramento de alerta, usamos os mesmos cálculos da função de callback do Dash
            # Não é necessário carregar o dataframe completo se o dado mais recente está disponível,
            # mas vamos manter a lógica de cálculo de acumulado de 72h aqui para precisão.
            ...
            if not df.empty and 'timestamp' in df.columns:
                try:
                    ...
                    # Trata o caso em que o DataFrame ainda está sendo construído ou tem erro de tipo
                    # print(f"Erro no cálculo do acumulado 72h no monitor: {e}")
                    pass
            ...
            print("Tarefa do monitor de alertas cancelada.")
            break
        except Exception as e:
...
async def lifespan(app: FastAPI):
    ...
    task_simulador = asyncio.create_task(rodar_simulador())
    task_monitor = asyncio.create_task(monitorar_alertas())

    yield

    task_simulador.cancel()
    task_monitor.cancel()
    print("Simulador e Monitor parados.")
...
dash_app.layout = dbc.Container([
    ...
    dbc.Row([dbc.Col(dbc.Card([dbc.CardHeader("Nível Operacional (Chuva 72h)", className="text-center fw-bold"),
                               dbc.CardBody(id='rain-alert-display', className="text-center")]), width=6, lg=4,
                     className="mb-3 mb-lg-0"), dbc.Col(dbc.Card(
        [dbc.CardHeader("Nível Operacional (Umidade Solo)", className="text-center fw-bold"),
         dbc.CardBody(id='soil-alert-display', className="text-center")]), width=6, lg=4), ],
            className="mb-4 justify-content-center"),
    dbc.Row([dbc.Col([dbc.Label("Período:", html_for="periodo-dropdown"), dcc.Dropdown(id='periodo-dropdown', options=[
        {'label': f"{h} hora{'s' if h > 1 else ''}", 'value': h} for h in
        [1, 3, 6, 12, 18, 24, 36, 48, 60, 72, 84, 96]], value=72, clearable=False, style={'marginBottom': '20px'})],
...
def calculate_soil_alert(current_data):
    ...
    if is_above_1m and is_above_2m and is_above_2m5:
        soil_alert_level, soil_alert_color = "Paralização", "red"
    elif is_above_2m and is_above_2m5:
...
def update_graphs(n_intervals, selected_hours):
    ...
    try:
        ...
        return fig_pluvia_default, fig_umidade_default, rain_alert_default, soil_alert_default

    # Cálculo do Alerta de Chuva (para o display)
    ...
    # Cálculo do Alerta de Umidade (para o display)
    soil_alert_level, soil_alert_color = calculate_soil_alert(data)

    # Conteúdo para os cards
    ...
    data_points_to_show = int(selected_hours) * PONTOS_POR_HORA;
    df_filtered = df.tail(data_points_to_show)
    ...
    df_filtered.loc[:, 'precipitacao_acumulada_recalculada'] = df_filtered['pluviometria_mm'].cumsum()
    fig_pluvia = make_subplots(specs=[[{"secondary_y": True}]])
    fig_pluvia.add_trace(go.Bar(x=df_filtered.index, y=df_filtered['pluviometria_mm'], name='Pluviometria (mm)',
                                marker_color='rgb(55, 83, 109)'), secondary_y=True, );
    fig_pluvia.add_trace(go.Scatter(x=df_filtered.index, y=df_filtered['precipitacao_acumulada_recalculada'],
    ...
    try:
        x_range_pluvia = [df_filtered.index.min(), df_filtered.index.max() + pd.Timedelta(hours=1)]
    except ValueError:
        x_range_pluvia = None
    ...
    fig_umidade.add_trace(
        go.Scatter(x=df_filtered.index, y=df_filtered['umidade_1m_perc'], name='Profundidade 1 m', mode='lines',
                   line=dict(color='#28a745', width=3)));
    fig_umidade.add_trace(
        go.Scatter(x=df_filtered.index, y=df_filtered['umidade_2m_perc'], name='Profundidade 2 m', mode='lines',
                   line=dict(color='#ffc107', width=3)));
    fig_umidade.add_trace(
        go.Scatter(x=df_filtered.index, y=df_filtered['umidade_2m5_perc'], name='Profundidade 2.5 m', mode='lines',
    ...
    fig_umidade.update_layout(title_text="Umidade Volumétrica do Solo", yaxis_title="Umidade Volumétrica (%)",
                              ...
                              yaxis_range=[UMIDADE_BASE_2M5 - 5, UMIDADE_SATURACAO + 5], plot_bgcolor='white',
                              paper_bgcolor='white',
                              legend=dict(orientation="h", yanchor="bottom", y=-0.3, xanchor="center", x=0.5))
    try:
        x_range_umidade = [df_filtered.index.min(), df_filtered.index.max() + pd.Timedelta(hours=1)]
    except ValueError:
        x_range_umidade = None
    fig_umidade.update_xaxes(showgrid=False, zeroline=False, range=x_range_umidade);
